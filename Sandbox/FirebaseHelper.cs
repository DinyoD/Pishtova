...
namespace Sandbox
{
    ...
    public class FirebaseHelper
    {
        ...
        {
            var subjectInfo = GetSubjectInfoFromFirebase(collectionName);
            var subjectName = collectionName.Split('_')[1];

            return new Subject
            {
                Name = subjectName,
                Categories = ExtractCategoriesInfo(subjectInfo)
            };
        }
        public ICollection<Category> ExtractCategoriesInfo(List<List<List<string>>> subjectInfo)
        {
        ...
        public Subject Get_BG_SubjectInfo(string collectionName)
        {
            var subjectInfo = Get_BG_SubjectInfoFromFirebase(collectionName);
            var subjectName = collectionName.Split('_')[1];

            return new Subject
            {
                Name = subjectName,
                Categories = Extract_BG_CategoriesInfo(subjectInfo)
            };
        }
        public ICollection<Category> Extract_BG_CategoriesInfo(Dictionary<string, List<List<string>>> subjectInfo)
        {
            ICollection<Category> categories = new List<Category>();
